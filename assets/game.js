...
var hangmanGame = {
    ...
    letterGuessed: null,
    wins: 0,
    // Setting up the game when it loads
    setupGame: function() {
    //   Selecting a random word from the above wordsToPick object
      var objKeys = Object.keys(this.wordsToPick);
      this.wordInPlay = objKeys[Math.floor(Math.random() * objKeys.length)];
      // Splitting the chosen word up into its individual letters.
      this.lettersOfTheWord = this.wordInPlay.split("");
    //   Builds first representation of the word, using the format: ("_ _ _ _ ...")
      this.rebuildWordView();
      // This function sets the number of guesses the user gets, and renders it to the HTML.
      this.processUpdateTotalGuesses();
    },
    // Function for every time a user selects a word
    updatePage: function(letter) {
      ...
      if (this.guessesLeft === 0) {
        ...
        // Managing incorrect guesses
        this.updateGuesses(letter);
        // Managing correct guesses.
        this.updateMatchedLetters(letter);
        // Rebuild the view of the word. Guessed letters are revealed, unguessed letters have a "_".
        this.rebuildWordView();
        // Function for when the user wins: restarting the game
        if (this.updateWins() === true) {
          this.restartGame();
        }
      }
    },
    // This function specifies what happens when the user makes a new incorrect guess.
